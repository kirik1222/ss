...
net.Receive("huysound",function(len)
    ...
    if not IsValid(ent) then return end  -- Проверка на валидность сущности

    if ent == LocalPlayer() then return end

    local dist = LocalPlayer():EyePos():Distance(pos)
    if dist < 4000 then
        ent:EmitSound(sound, 75, math.random(90, 110), 1, CHAN_WEAPON)
    else
        timer.Simple(dist / 45000, function()
            if IsValid(ent) then  -- Проверка на валидность сущности перед использованием
                ent:EmitSound(farsound, 85, math.random(90, 110), 1, CHAN_WEAPON)
            end
        end)
    end
end)
